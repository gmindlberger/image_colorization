...
class Trainer:
    ...
    def show_examples(self, loader=None, n_display=3):
        # Default: Validation oder Training
        if loader is None:
            loader = self.val_loader if self.validate and self.val_loader else self.train_loader

        self.model.eval()
        with torch.no_grad():
            L_batch, AB_batch = next(iter(loader))  # Eine Batch ziehen
